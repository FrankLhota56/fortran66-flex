 ...
 *
 * Cards are of fixed length, usually 80 characters. When successful, this
 * function will store in `card` exactly `cardlen` characters, followed by a
 * newline character and a null. Make sure that `card` can hold at least
 * `cardlen`+2 characters.
 *
 * The tab character is not supported by some older programming languages, so
 * this function can replace tabs with spaces.
 *
 ...
 */
extern char *getcard(char *card, size_t cardlen, size_t tablen, FILE *in);
